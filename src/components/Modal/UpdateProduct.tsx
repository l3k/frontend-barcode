...
const AddProduct: React.FC<AddProductProps> = ({ product, showModal, onCloseModal, handleSave }) => {
  ...
  async function newProduct(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await updateProduct(product.id, code, name, description, currency, token)
      onCloseModal()
      setCode('')
      setName('')
      setDescription('')
